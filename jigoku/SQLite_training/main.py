...
from .print_std import *  # @UnresolvedImport

class Card:
    # 各カードの持つ属性：スート、数字、強さ（弱2,3,...,Q,K,A強）
...
class Hand:
    # 各プレーヤーの手札。コンストラクタでは引数のカードを受け取りスートごと、強さ順にソート
    def __init__(self, cards):
        self.cards = sorted(cards, reverse=True, key=attrgetter('suit', 'rank'))
    # カードを捨てる（場に出す）。手札からはなくなる
    def discard(self, num):
        return self.cards.pop(num)

class Field:
    # 場にあるカード。コンストラクタではその回のディーラー（親）の情報を保存
...
def eachturn_setup(dealer, my_hand, me):
    # ターンの最初にする処理。自分の手札の表示とディーラー（親）の表示
    print_hand(my_hand)
    if dealer == me:
        print("dealer:You")
    else:
        ...
        if len(hand.cards)==1:
            ...
            while True:
                ...
                if not(this_suit and len([x for x in hand.cards if x.suit==this_suit])!=0 and this_suit!=hand.cards[chosen_card].suit):
                    break
    return hand.discard(chosen_card)
...
def culc_result(player_amounts, player_pred, player_num, me, player_points):
    ...
    for player in range(player_num):
        ...
        if player_amounts[player]==player_pred[player]:
            ...
            print("{} failed as 0!! -5".format(player_str))
